...
After a successful run, you can export a reproducibility evidence bundle that captures the complete provenance of your experiment:

```bash
# Export evidence for a completed run
matterstack export-evidence <run_id>
```

### Evidence Bundle Contents

The export creates an `evidence/` directory containing:

| File | Description |
|------|-------------|
| `bundle.json` | Complete run metadata including task results, timing, and operator wiring |
| `report.md` | Human-readable summary of the run with key metrics |
| `operators_snapshot/` | Copy of `operators.yaml` configuration used during execution |

### Example Usage

```bash
# Run the thin-film lab demo
python main.py

# List completed runs
matterstack list demos/thin_film_lab

# Export evidence for reproducibility
matterstack export-evidence run_20241220_143052

# The evidence bundle will be created at:
# workspaces/demos/thin_film_lab/runs/run_20241220_143052/evidence/
```
...
- **Operator Wiring Provenance**: Which `operators.yaml` was used and its contents
- **Task Execution Details**: Start/end times, status, and outputs for each task
- **Sim-to-Real Metrics**: The reconciliation data comparing predictions to experimental results
- **Environment Snapshot**: MatterStack version and configuration context

This enables full reproducibility and audit trails for autonomous lab experiments.