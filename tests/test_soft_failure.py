...
def test_workflow_soft_failure(tmp_path):
    ...
    assert result.status == JobState.PARTIAL_SUCCESS
    
    # A should be COMPLETED
    assert result.tasks["task_a"].status.state == JobState.COMPLETED
    
    # B should be FAILED
    assert result.tasks["task_b"].status.state == JobState.FAILED
    ...
    assert result.tasks["task_d"].status.state == JobState.COMPLETED
...
def test_workflow_strict_abort(tmp_path):
    ...
    assert result.tasks["task_a"].status.state == JobState.FAILED
