...
## ⚠️ Important: Setup for Modern Python (3.10+)

When working with modern Python features (e.g., `match` statements, `typing.list`, `subprocess.run(capture_output=True)`), using the system `module load python` is **NOT SUPPORTED** and will result in errors like:
* `Lmod has detected the following error: The following module(s) are unknown: "python"`
* `/var/spool/slurmd/.../slurm_script: line 19: python: command not found`
* `sacct ... FAILED|127:0` (Exit code 127 = Command Not Found)

### Correct Slurm Job Setup
To properly execute modern Python code in a Slurm job, you must:

1.  **Load Anaconda or Miniforge**: This provides the base Python environment and Conda/Mamba tools.
2.  **Activate Conda Hook**: For non-interactive batch scripts, you explicitly need to initialize the shell hook before activating an environment.
...
# 1. Purge and Load Anaconda (NOT python)
module purge
module load anaconda

# 2. Initialize Conda for Batch Shell (CRITICAL STEP)
eval "$(conda shell.bash hook)"

# 3. Activate Environment (Optional: 'base' is active by default after load)
...
### Creating a Custom Python 3.10+ Environment
If the base environment is not sufficient, create a custom environment on an interactive compile node (`acompile`):
...
# 2. Create Environment (e.g., Python 3.11)
conda create -n modern_py python=3.11 -y

# 3. Activate and Install Packages
conda activate modern_py
conda install -y numpy scipy pandas
```

Then, in your Slurm script (as shown above), simply run `conda activate modern_py` after the hook initialization.

### TL;DR Summary
*   **DO NOT** run `module load python` (It does not exist).
*   **DO** run `module load anaconda`.
*   **ALWAYS** add `eval "$(conda shell.bash hook)"` before `conda activate` in batch scripts.
*   **VERIFY** with `python --version` in your script to ensure you aren't picking up a legacy system python.
