...
def initialize_run(
    workspace_slug: str, campaign: Campaign, base_path: Path = Path("workspaces"), run_id: Optional[str] = None
) -> RunHandle:
    """
    ...
    handle = RunHandle(workspace_slug=workspace_slug, run_id=run_id, root_path=root_path)
...
def initialize_or_resume_run(
    ...
    if resume_run_id:
        ...
        if target_path.exists():
            ...
            return RunHandle(workspace_slug=workspace_slug, run_id=resume_run_id, root_path=target_path)
        else:
    ...
    try:
        ...
        if status in active_statuses:
            ...
            return RunHandle(workspace_slug=workspace_slug, run_id=latest_run_id, root_path=latest_run_dir)
        elif resume_always:
