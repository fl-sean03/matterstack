...
class CampaignMonitor:
    ...
    def generate_header(self) -> Panel:
        ...
        # Color code status
        style = "green" if status == "RUNNING" else "yellow" if status == "PENDING" else "red" if status == "FAILED" else "blue"
        ...
        grid.add_row(
            f"Workspace: [bold]{self.handle.workspace_slug}[/bold]",
            f"Run ID: [bold]{self.handle.run_id}[/bold]"
        )
        
        return Panel(
            grid,
            title=f"MatterStack Mission Control - [{style}]{status}[/{style}]",
            border_style=style
        )
    ...
    def generate_task_table(self) -> Table:
        ...
        for task in tasks:
            ...
            elif status == "RUNNING": status_style = "yellow"
            elif status == "WAITING_EXTERNAL": status_style = "magenta"
